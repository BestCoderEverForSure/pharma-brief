# 🎤 60-second demo script (gallery walk)

Have the website open (Command Centre → Read the brief → localhost, or just open `site/public/index.html`).

**(0–10s) The problem — name the user.**
...
> "Same recipe, two interchangeable AI engines" *(point to the badges in the archive)*. "Claude for the richest read, and a cheap DeepSeek pipeline that runs **automatically in the cloud every morning** — so it emails me even when my laptop is closed."

**(45–60s) Prove it's real, then the 'so what'.**
> "This isn't a mock-up — here's the GitHub Actions run that generated and emailed it" *(EVIDENCE.md, or the Actions tab green ✓)* "and the live website it publishes. The point: it turns ~45 minutes of scanning into a 2-minute, fact-checked brief — and the same template could cover any fast-moving industry, like AI."
...
## For peer reviewers (one screen)
Open **`OPEN ME FIRST.md`** → it points to the website (try it), the **email preview** (`samples/email-preview.html`), and **`EVIDENCE.md`** (proof it runs). No setup needed.

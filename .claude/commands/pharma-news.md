...
- **`morning`** *(default)* → a genuinely tight **2-3 minute** read. **HARD CAP: ≤ ~550 words total.** Include ONLY: the **Talking point**, a short **TL;DR**, **at most 3 Top Stories** (each "What it means" 2-3 sentences max, never padded), and a **brief Eli Lilly Spotlight** (2-3 bullets). **OMIT the "On the radar" and "Wider world → pharma" sections** in the morning — they're evening-only, and upcoming items are auto-appended as "Upcoming catalysts". Don't add other sections. If it won't fit, cut the least-important story — a tight brief that gets read beats a thorough one that doesn't. This is the point of the morning edition; honour it over any depth instinct below.

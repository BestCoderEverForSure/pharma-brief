...
- The website footer shows a **"Last updated"** time, so a stalled run is visible at a glance. Critical failures (feeds down, thin digest, a failed send/publish) exit non-zero, so GitHub emails you the failure rather than failing silently. If the engine is **rate-limited or overloaded** (a common free-tier "model is overloaded" 503), the run retries it for ~50s. An optional **cross-engine fallback** (Gemini↔DeepSeek) can then take over so the brief still ships — but it's **off by default** (toggle it in the Command Centre under *Engine fallback*, or set the `PHARMA_ENGINE_FALLBACK` repo Variable). By default the run stays on your chosen engine and emails you if it's down, rather than spending on the paid engine.
...
**Website (local):** `python3 site/build_site.py` renders every digest into a styled, browsable site at `site/public/index.html` - with an archive grid (engine badges), a forward-looking **catalyst timeline** (a category-mix row above the dated list), a markets strip whose %-move matches each brief's window (daily ~5-day, weekly ~7-day), and a one-click **▶ Listen** read-aloud on each brief (browser speech synthesis; skips citations and metadata). It also emits a subscribe-able **RSS feed** (`feed.xml`, auto-discovered by feed readers) and a subscribe-able **catalyst calendar** (`catalysts.ics`): click **Add to your calendar** in the Catalysts section to import the upcoming events, or subscribe your calendar app to `https://<your-pages-url>/catalysts.ics` so PDUFA dates, earnings, and readouts stay current with reminders. The archive index caps at the 30 most recent and links to a full `archive.html`, and a **Threads** view (`threads.html`) lets you follow a company, drug, or theme across the archive — newest-first, with snippets. (Instant search + Threads run off `search-data.js`, which is capped at the 500 most recent briefs so it can't grow without bound — generously over a year of dailies; older briefs stay reachable via the full archive and their own pages.) It opens with a **coverage chart** (your `watchlist.md` topics ranked by how many briefs mention them), and clicking a topic opens its storyline plus a **weekly-mentions trend** (tidy columns that get richer as the archive deepens); free text covers anything else. Topics match common **synonyms** too (e.g. "Lilly" counts toward Eli Lilly, "merger"/"acquisition" toward M&A — edit the `TOPIC_ALIASES` map in `build_site.py`). The site is also an **installable PWA**: a web app manifest + service worker mean you can "Add to Home Screen" on a phone and read/listen **offline** (the shell and recent briefs are precached). Both engines rebuild it automatically after each run. Open `index.html` in any browser.
...
2. Add repo **Secrets**: `GEMINI_API_KEY`, `RESEND_API_KEY`, `EMAIL_TO`, `EMAIL_FROM` (and, to enable each option: `DEEPSEEK_API_KEY` + `DEEPSEEK_MODEL` for the fallback engine, `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` for Telegram, `SITE_URL` for absolute RSS links). Add repo **Variables**: `PHARMA_ENGINE` (`gemini` | `deepseek`), `GEMINI_MODEL` (default `gemini-2.5-flash`; note `DEEPSEEK_MODEL` is a Secret, not a Variable), and optionally `PHARMA_ENGINE_FALLBACK` (`1` to let a busy/overloaded engine fall back to the other one; off by default).
...
- ✅ On-demand + cloud-scheduled digest · email · **Telegram** · evening + Sunday weekly editions · audio · website (markets, catalyst timeline + category mix, **calendar subscribe** `.ics`, **▶ Listen** read-aloud, **Threads** topic view + **coverage** chart + per-topic **weekly trend**, **installable PWA + offline**, search, dark mode, "last updated") · thread-tracking / "what's new" tagging · archive search · accuracy pass + **grounding self-check** · self-maintaining catalysts · source date/times · DeepSeek engine

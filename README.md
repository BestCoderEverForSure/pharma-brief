...
> **First-time note:** new slash commands only appear in the menu after you **restart Claude Code**. The automatic morning run does **not** need this (see §6).
...
The DeepSeek version (§8) is grounded *only* on the articles it fetches — the lowest-hallucination setup.
...
**To make the 7am run reliable:** leave the Mac plugged in, awake, and the Claude app open overnight. For a digest that arrives **even if your laptop is closed**, run the **DeepSeek version on an always-on machine** (§8) — that's the only setup fully independent of this laptop.
...
| Morning run didn't fire at 7am | Mac was asleep/closed or app shut — see §6. |

...
- [x] **Listen fix + longevity pass (2026-06-30)** - the on-page **▶ Listen** control now reliably **resumes where you paused** (it remembers the spot itself instead of leaning on the browser's flaky native `pause()`/`resume()`), plus a **restart** button to start a brief over from the top; **capped `search-data.js`** (the client search/Threads index) at the 500 most recent briefs so it can't grow without bound (older briefs stay on the site via the archive). A read-only **resilience audit** confirmed the generation/delivery pipeline already fails cleanly on every mode (missing key, thin feeds, model 429/network errors, empty output, grounding/delivery failures) and that every other growth surface is already bounded (precache 12, feed 20, archive 30, `state.json` history 60).
- [x] **Engine auto-fallback on overload (2026-06-30)** - the recurring "Gemini servers busy" (503) that used to fail the run and email an error is now ridden out: each model call retries transient 429/503/network blips more patiently (~50s over 4 tries), and if the primary engine *stays* down, the whole call transparently retries on the **other configured engine** (`call_model_with_fallback`) — so a busy Gemini falls back to DeepSeek and the brief still ships on time. The run only exits non-zero (and emails you) if **every** keyed engine is down. Needs the other engine's key set (e.g. `DEEPSEEK_API_KEY` as a repo Secret) for the fallback to engage; covered by new offline tests.

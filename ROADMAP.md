...
- [x] **More patient model retries + opt-in engine fallback (2026-06-30)** - the recurring "Gemini servers busy" (503) is first ridden out by more patient retries on the *same* engine (~50s over 4 tries; free). Beyond that, an **opt-in** cross-engine fallback (`call_model_with_fallback`) can regenerate on the other configured engine so the brief still ships — but it's **OFF by default**: a sustained Gemini outage fails loudly and emails you rather than silently spending on the paid engine. Turn it on in the Command Centre ("Engine fallback") or via the `PHARMA_ENGINE_FALLBACK` repo Variable (needs the other engine's key set, e.g. `DEEPSEEK_API_KEY`). Covered by new offline tests.

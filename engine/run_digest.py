...
def build_system_prompt(edition: str, engine_label: str = "DeepSeek", mode: str = "daily") -> str:
    ...
    parts = ["You are generating the Pharma Digest.",
             ...
             *( ["- LENGTH — HARD CAP (this is the everyday morning brief, often read in a rush): keep the ENTIRE digest at or under ~550 words, excluding the auto-appended Sources/catalysts/markets — a true coffee-length 2-3 minute read. The morning edition contains ONLY: the Talking point, a short TL;DR, AT MOST 3 Top Stories (each 'What it means' 2-3 sentences MAX, never padded), and a brief Eli Lilly Spotlight (2-3 bullets). OMIT the 'On the radar' and 'Wider world → pharma' sections entirely — those are evening-only (forward-looking items are already auto-appended as Upcoming catalysts). Do NOT add other ## sections. If it won't fit, drop the least-important story — a tight brief that gets read beats a thorough one that doesn't. This OVERRIDES any larger structure or section count in the methodology/template below."] if tight else [] ),
